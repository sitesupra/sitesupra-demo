...
use Supra\Social\AdapterAbstraction;
use Supra\Social\Exception\SocialMediaException;
...
class Adapter extends AdapterAbstraction
{
	 ...
	 */
	public function addTabToPage(UserFacebookPage $page, $tabTitle = null)
	{
		$this->toggleTabOnPage($page, $tabTitle, 'add');
	}
	 ...
	 * @param UserFacebookPage $page 
	 */
	public function removeTabFromPage(UserFacebookPage $page)
	{
		$this->toggleTabOnPage($page, null, 'remove');
	}

	/**
	 * Adds or removes tab from facebook page
	 * @param UserFacebookPageTab $page
	 * @param string $action add/remove
	 */
	private function toggleTabOnPage(UserFacebookPage $page, $title = null, $action = 'remove')
	{
		if(empty($title)) {
			$title = 'SiteSupra Tab';
		}
		...
		if ($action == 'add') {
			...
			$this->instance->api('/me/tabs/app_' . $this->getAppId(), 'POST', array('access_token' => $pageAccessToken, 'custom_name' => $title));
		} else {
