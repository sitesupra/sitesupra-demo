...
class ThemeSetActiveCommand extends Command
{
	...
	{
		$this->setName('su:themes:set_active')
				->setDescription('Set active theme')
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$theme = $themeProvider->getThemeByName($themeName);
		
		$themeProvider->setActiveTheme($theme);
		
		$eventManager = ObjectRepository::getEventManager();
		
		$args = new FrontControllerShutdownEventArgs();
		
		$eventManager->fire(FrontControllerShutdownEventArgs::frontControllerShutdownEvent, $args);
		
		$output->writeln('Theme "' . $themeName . '" set as active.');
	}
