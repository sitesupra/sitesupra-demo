YUI.add('gallery.view-highlight', function (Y) {
	//Invoke strict mode
	"use strict";
	...
	//Template
	var TEMPLATE_HIGHLIGHT_T = '<div class="yui3-inline-reset yui3-box-reset supra-gallery-highlight-top"></div>',
		TEMPLATE_HIGHLIGHT_R = '<div class="yui3-inline-reset yui3-box-reset supra-gallery-highlight-right"></div>',
		TEMPLATE_HIGHLIGHT_B = '<div class="yui3-inline-reset yui3-box-reset supra-gallery-highlight-bottom"></div>',
		TEMPLATE_HIGHLIGHT_L = '<div class="yui3-inline-reset yui3-box-reset supra-gallery-highlight-left"></div>';
	
	/*
	 * Editable content
	 */
	function ViewHighlight (config) {
		ViewHighlight.superclass.constructor.apply(this, arguments);
	}
	
	ViewHighlight.NAME = 'gallery-itemlist-highlight';
	ViewHighlight.NS = 'highlight';
	...
	Y.extend(ViewHighlight, Y.Plugin.Base, {
		...
		reattachListeners: function () {
			...
			if (view.order) {
				view.order.on('dragStart', this.hideHighlight, this);
			}
			
			view.on('focusItem', this.hideHighlight, this);
			view.on('addItem', this.showHighlight, this); // updates position
			
			childSelector = view.getItemCSSSelector();
			...
			Y.Node(view.getDocument().body).append(nodes);
		...
		mouseEnter: function (evt) {
			...
			if (this.get('host').isEditing()) {
				// Don't show highlight while editing
				return;
			}
			if (!target || target.hasClass('supra-gallery-new')) {
		...
		showHighlight: function (target, showMessage) {
			...
			if (showMessage) {
				...
				nodes.item(2).set('innerHTML', '<span class="yui3-inline-reset yui3-box-reset">' + Supra.Intl.get(['gallery', 'edit_move']) + '<span class="yui3-inline-reset yui3-box-reset"></span></span>');
	...
	Supra.GalleryViewHighlight = ViewHighlight;
