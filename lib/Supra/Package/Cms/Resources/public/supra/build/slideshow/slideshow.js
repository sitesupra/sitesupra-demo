YUI.add('supra.slideshow', function (Y) {
	...
	Y.extend(Slideshow, Y.Widget, {
		...
		/**
		 * Currently UI sync is in progress
		 */
		...
		bindUI: function () {
			...
			if (layout) this.listeners.push(layout.after('contentresize', this.syncUI, this));
		...
		syncUI: function () {
			...
			this.slide_width = null; // clear cache
		...
		addSlide: function (_options) {
			var options = Supra.mix({
				...
				'title': '',
				'icon': ''
			}, Y.Lang.isObject(_options) ? _options : {'id': _options});
