YUI().add('supra.google-fonts', function (Y) {
	...
	Y.extend(GoogleFonts, Y.Base, {
		...
		getLinkNodes: function () {
			return GoogleFonts.getLinkNodes(this.get('doc'));
		},
		 ...
		 * @private
		 */
		_setFonts: function (_fonts) {
			var fonts = (this.get('fonts') || []).concat(_fonts),
				i = 0,
				ii = fonts.length,
				unique_arr = [],
				unique_hash = {},
	 ...
	 * Returns link node
	 * 
	 * @param {Object} doc Document element inside which to look
	 * @returns {Array} Google fonts link nodes
	 */
	GoogleFonts.getLinkNodes = function  (doc) {
		var head = null,
			links = null;
	...
	GoogleFonts.loadFonts = function () {
		...
		var deferred = new Supra.Deferred(),
			...
			uri      = Supra.Url.generate('cms_pages_fonts_google_list');
			...
			function (fonts) {
				...
				for (; i<ii; i++) {
					...
					formatted.push({
						'title': fonts[i],
						'family': family,
						'apis': fonts[i].replace(/ /g, '+') + ':300,300italic,regular,italic,700,700italic'
					});
				}
