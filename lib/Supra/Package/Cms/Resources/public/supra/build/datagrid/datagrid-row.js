	...
	DataGridRow.prototype = {
		...
		getID: function () {
			...
			if (!id) {
				...
				if (!id) {
					id = +new Date() + '-' + Math.round(Math.random() * 60000);
				}
			}
		...
		getColumnNode: function (column_id) {
			...
			return this.nodes[column_id] = this.node.one('td[data-column-id="' + column_id + '"]');
		},
		
		/**
		 * Returns column title from element
		 * 
		 * @param {Object} node Node
		 * @returns {String} Column id or null 
		 */
		getColumnIdFromNode: function (node) {
			if (!node || !this.node) return null;
			if (!(node = node.closest('td'))) return null;
			...
			for (; i<ii; i++) {
				if (node.test('[data-column-id="' + columns[i].id + '"]')) {
					return columns[i].id;
				}
			}
		},
		...
		getNode: function () {
			...
			for(var i=0,ii=columns.length; i<ii; i++) {
				...
				classname = column_id.replace(/[^a-z0-9\-_]*/ig, '');
				classname = classname ? 'row-' + classname.toLowerCase() : '';
				...
				html[html.length] = '<td ' + (column.width ? 'width="' + column.width + '" ' : '') + 'class="' + classname + align + '" data-column-id="' + column_id + '">' + value + '</td>';
			}
