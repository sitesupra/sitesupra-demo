YUI.add('supra.input-proto', function (Y) {
	...
	Y.extend(Input, Y.Widget, {
		...
		renderUI: function () {
			...
			//Add classnames
			bound.addClass(this.getClassName());
		...
		showError: function (message) {
			if (typeof message === 'string') {
				...
				if (!node) {
					node = Y.Node.create('<div class="su-input-error-message"></div>');
		...
		_setError: function (error) {
			this.get('boundingBox').toggleClass('su-input-error', error);
		...
		_setDisabled: function (value) {
			...
			this.get('boundingBox').toggleClass('su-input-disabled', value);
		...
		_setStyle: function (style) {
			...
			if (prev) {
				this.removeClass(this.getClassName(prev));
			}
			if (style) {
				this.addClass(this.getClassName(style));
			}
		...
		/**
		 * Returns hierarhical input name
		 * 
		 * @returns {String} Name
		 */
		getHierarhicalName: function () {
			var parent = this.get('parent'),
				name   = [this.get('name') || this.get('id')],
				input  = parent;
			
			while (input && input.isInstanceOf('input')) {
				parent = input.get('parent');
				
				if (!parent.isInstanceOf('collection')) {
					name.unshift(input.get('name') || this.get('id'));
				}
	...
	Supra.Input = {
		...
		'isInline': function (type, default_value) {
			var inline = (default_value === true || default_value === false ? default_value : false);
			
			if (type && typeof type === 'object') {
				if (type.constructor.IS_INLINE) inline = true;
			} else {
		...
		'isContained': function (type, default_value) {
			var contained = (default_value === true || default_value === false ? default_value : true);
			
			if (type && typeof type === 'object') {
				if (type.constructor.IS_CONTAINED === false) contained = false;
			} else {
				var type_str = String(type || ''),
					name = type_str.substr(0, 1).toUpperCase() + type_str.substr(1);
		 ...
		 * Value is taken from 'defaultValue' attribute
		 *
		 * @param {String} type Input type
		 * @returns {Object|String|Number|Null} Default value or null
		 */
		'getDefaultValue': function (type) {
			var type_str = String(type || ''),
				name = type_str.substr(0, 1).toUpperCase() + type_str.substr(1),
				constructor = Supra.Input[name],
				value = null;
			
			if (constructor) {
				value = constructor.ATTRS.defaultValue;
				
				while (value == null && constructor && constructor.superclass) {
			...
			if (value) {
				if (value.value && typeof value.value === 'object') {
					if (Y.Lang.isArray(value.value)) {
						// Array
						return Supra.mix([], value.value, true);
					} else {
						// Object
						return Supra.mix({}, value.value, true);
					}
