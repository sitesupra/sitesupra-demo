YUI().add('supra.htmleditor-plugin-table', function (Y) {
	...
	var HTMLEDITOR_COMMAND = 'inserttable',
		...
		HTMLEDITOR_BUTTONS = [HTMLEDITOR_BUTTON, 'row-before', 'row-delete', 'row-after', 'merge', 'column-before', 'column-delete', 'column-after', 'table-settings'];
	...
	Supra.HTMLEditor.addPlugin('table', defaultConfiguration, {
		...
		cmdRowDelete: function () {
			...
			this.selected_cell.addClass('su-cell-selected');
		...
		cmdColBefore: function () {
			var history = this.htmleditor.getPlugin('history'),
				...
				sel_td = Y.Node.getDOMNode(this.selected_cell),
		...
		setProperty: function (id, value) {
			if (id == 'rows') {
				...
				if (this.selected_cell) {
					this.selected_cell.removeClass('su-cell-selected');
				}
		...
		focusTable: function (element) {
			...
			if (this.selected_table && (!table || !table.compareTo(this.selected_table))) {
				this.selected_table.removeClass('su-table-selected');
			}
			if (table) {
				table.addClass('su-table-selected');
			}
			...
			if (cell) {
				cell.addClass('su-cell-selected');
			}
		...
		insertTable: function (values) {
			...
			if (!htmleditor.get('disabled') && htmleditor.isSelectionEditable(htmleditor.getSelection())) {
				...
				var html_row = '<tr><td>' + cell_html + '</td><td>' + cell_html + '</td><td>' + cell_html + '</td></tr>',
					html_table = '<table class="hidden-xs hidden-sm"><tbody><tr><th>' + cell_html + '</th><th>' + cell_html + '</th><th>' + cell_html + '</th></tr>' + html_row + html_row + '</tbody></table>';
		...
		onNodeChange: function () {
			var htmleditor = this.htmleditor,
				element = htmleditor.getSelectedElement('img,svg,td,th,table'),
			...
			if (isSpecial) {
				...
				// Enable if editing is allowed
				this.toggleButtons(allowEditing);
			}
		},
		...
		init: function (htmleditor) {
			...
			if (button) {
				...
				htmleditor.on('editingAllowedChange', function (event) {
					...
					this.toggleButtons(event.allowed);
				}, this);
		...
		toggleButtons: function (enabled) {
			var buttons = HTMLEDITOR_BUTTONS,
				toolbar = this.htmleditor.get('toolbar'),
				i = 0,
				ii = buttons.length;
				
			if (toolbar) {
				for (; i<ii; i++) {
					toolbar.getButton(buttons[i]).set('disabled', !enabled);
				}
			}
		},
