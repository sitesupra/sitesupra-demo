YUI.add("supra.footer", function (Y) {
	...
	Footer.NAME = "footer";
	Footer.CSS_PREFIX = 'su-' + Footer.NAME;
	...
	Y.extend(Footer, Y.Widget, {
		...
		renderUI: function () {
			...
			//Style
			this.get("srcNode").addClass(this.getClassName(this.get("style")));
		...
		normalizeButtonConfig: function (config) {
			...
			//Merge, we do this manually, because we
			//need to ignore empty strings
			var i = 1,
				ii = args.length,
				dest = args[0],
				key, val;
