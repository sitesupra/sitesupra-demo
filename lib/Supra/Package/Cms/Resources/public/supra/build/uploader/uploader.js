...
YUI.add('supra.uploader', function (Y) {
	...
	Y.extend(Uploader, Y.Base, {
		...
		createInput: function () {
			...
			input = Y.Node.create('<input autocomplete="off" class="offscreen" type="file" ' + (multiple ? 'multiple="multiple" ' : '') + (accept ? 'accept="' + accept + '" ' : '') + '/>');
		...
		createLegacyInput: function (container, for_replace) {
			var uri = document.location.protocol + '//' + document.location.hostname + '/public/cms/supra/build/io/blank.html',
				node_id = Y.guid(),
				iframe,
				form,
				input;
			...
			form   = Y.Node.create('<form autocomplete="off" target="' + node_id + '" class="legacy-file-upload-form" method="post" action="" enctype="multipart/form-data">' +
										'<input data-supra-ignore="true" type="file" name="file" class="upload-file-input" autocomplete="off" />' +
		...
		dragOver: function (evt) {
			...
			if (folder_id !== this.last_drop_id) {
				if (this.last_drop_target) {
					this.last_drop_target.removeClass('su-html5-dd-target');
				}
				if (folder_node) {
					folder_node.addClass('su-html5-dd-target');
				}
