...
YUI.add('supra.medialibrary-list-extended', function (Y) {
	...
	Extended.CLASS_NAME = Extended.CSS_PREFIX = 'su-' + Extended.NAME;
	...
	Extended.TEMPLATE_FILE = Template.compile('\
		<div class="file">\
			...
			{% if metaProperties|length %}\
				<div class="meta su-form-vertical"></div>\
			{% endif %}\
			\
			<div class="group">\
				<div class="info">\
	...
	Y.extend(Extended, List, {
		...
		renderUI: function () {
			...
			container.addClass(this.getClassName('extended'));
		...
		reloadImageSource: function (data) {
			...
			if (img_node) {
				...
				if (data.sizes && preview_size in data.sizes) {
					...
					src = "/public/cms/supra/build/medialibrary/assets/skins/supra/images/icons/file";
		...
		handleItemRender: function (event /* Event */) {
			if (event.type == List.TYPE_IMAGE || event.type == List.TYPE_FILE) {
				...
				node.all('input,textarea,select').each(function (item) {
					...
					obj.on('change', this.edit.onItemPropertyChange, this.edit, {'data': event.data, 'input': obj, 'type': 'native'});
		...
		renderMetaDataProperties: function (event, node) {
			...
			for (; i<ii; i++) {
				...
				// Handle value change
				input.on('change', this.edit.onItemPropertyChange, this.edit, {'data': event.data, 'input': input, 'type': 'meta'});
