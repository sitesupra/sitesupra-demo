...
YUI.add('supra.datagrid-draggable', function (Y) {
	...
	Y.extend(DraggablePlugin, Y.Plugin.Base, {
		...
		decorateProxy: function (e) {
			...
			var content = td.get('childNodes').item(0),
				drag_node = e.target.get('dragNode');
			
			if (content) {
				content = content.cloneNode(true);
			} else {
				content = document.createTextNode(td.get('innerText'));
			}
		...
		newItemDragOver: function (e) {
			...
			if (this.prependTimer) {
				this.prependTimer.cancel();
		...
		newItemDragOverMain: function (e) {
			...
			if (drop) {
				...
				if (!drag.get('parentNode') && !this.prependTimer) {
					// Wait just a little bit, because newItemDragOver may be called right after this
					// and the UI will flicker
					this.prependTimer = Y.later(1, this, function () {
						this.prependTimer = null;
						drop.prepend(drag);
