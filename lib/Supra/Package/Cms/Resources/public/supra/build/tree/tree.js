YUI.add('supra.tree', function(Y) {
	 ...
	 * 					<span class="img"><img src="/public/cms/supra/img/tree/home.png" /></span> <label>Home</label>
	 * 				</div>
	 * 				<ul class="tree-children">
	 * 					<li id="tree0_node1">
	 *		 				<div class="tree-node tree-node-scheduled">
	 * 							<span class="img"><img src="/public/cms/supra/img/tree/page.png" /></span> <label>Page</label>
	 * 
	 * 						</div>
	 *					</li>
	 * 				</ul>
	...
	Tree.NAME = 'Tree';
	Tree.CSS_PREFIX = Tree.CLASS_NAME = 'su-tree';
	...
	Supra.Tree = Y.Base.create('tree', Tree, [Y.WidgetParent], {
		...
		renderUI: function() {
			...
			this._fireContentResizeDelayed = Supra.throttle(this._fireContentResize, 50, this, true);
			this.on('toggle', this._fireContentResizeDelayed);
		},
		...
		_renderTreeUIChild: function (data, i) {
			this.add({'data': data, 'label': data.title, 'description': data.description, 'icon': data.icon}, i);
		},
		
		_fireContentResize: function () {
		...
		_setSelectedNode: function (node) {
			// Prevent all node selection
			if (!this.get('nodesSelectable')) return;
		...
		onDataLoad: function (data, status) {
			...
			this._fireContentResizeDelayed();
		},
		 ...
		 */
		showTooltip: function (description, node) {
			var tooltip = this.tooltip;

			if (!tooltip) {
				tooltip = this.tooltip = new Supra.Tooltip({
					'alignPosition': 'L',
					'zIndex': 10,
					'style':'dark'
				});
			...
			tooltip.set('htmlMessage', description);
			tooltip.set('alignTarget', node);
			tooltip.show();
		},
