...
YUI.add('supra.medialibrary-list', function (Y) {
	...
	List.NAME = 'medialist';
	List.CLASS_NAME = List.CSS_PREFIX = 'su-' + List.NAME;
	...
	List.TEMPLATE_FOLDER_ITEM_IMAGE = Template.compile('\
		<li class="type-image {% if broken or !thumbnail %}type-broken{% endif %}" data-id="{{ id }}">\
			<a>{% if !broken and thumbnail %}<img src="{{ thumbnail|escape }}?t={{ timestamp }}" alt="" draggable="false" />{% endif %}</a>\
			<span>{{ filename|escape }}</span>\
		</li>');
	...
	Y.extend(List, Y.Widget, {
		 ...
		 */
		setPrivateState: function (id, value) {
			var item = null,
				data = this.get('data');
			...
			value = Number(value);
			if (value == item.private) return this;
			...
			this.updatePrivateStateUI(id, value);
			
			//Update data
			data.save({'id': id, 'private': value}).fail(function () {
