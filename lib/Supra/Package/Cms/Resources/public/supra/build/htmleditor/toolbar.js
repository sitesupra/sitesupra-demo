YUI().add('supra.htmleditor-toolbar', function (Y) {
	...
	var TEMPLATE_GROUP = '<div class="su-editor-toolbar-{id} hidden">\
							<div class="su-editor-toolbar-{id}-content"></div>\
						  </div>';
	...
	HTMLEditorToolbar.CLASS_NAME = HTMLEditorToolbar.CSS_PREFIX = 'su-' + HTMLEditorToolbar.NAME;
	...
	Y.extend(HTMLEditorToolbar, Y.Widget, {
		...
		addControl: function (group_id, data, options) {
			...
			switch(data.type) {
				case 'separator':
					node = Y.Node.create('<div class="su-toolbar-separator"></div>');
					cont.append(node);
					break;
				case 'dropdown':
					...
					node.render();
					node.get('innerNode').setAttribute('title', title);
					break;
				case 'button':
				default:
					title = data.title || Y.Escape.html(Supra.Intl.get(['htmleditor', data.id]));
					...
					node.on('click', function (evt, config) {
						if (!node.get('disabled')) {
							if (config.command) {
								this.fire('command', config);
							} else if (config.action && Supra.Manager) {
								var action = Supra.Manager.getAction(config.action),
									button = node;
								...
								if (config.actionFunction) {
									if (action.NAME) {
										if (action.get('executed')) {
											//Call function
											action[config.actionFunction](config.id, config, button);
										} else {
											if (button) {
		...
		showGroup: function (group_id, silent) {
			...
			if (group) {
				if (!group.visible) {
					...
					position = this.getGroupPosition(group_id);
					group.node.setStyle('top', position + 'px');
				}
		...
		hideGroup: function (group_id, silent, after) {
			...
			if (group && group.visible) {
				groupOrder.splice(Y.Array.indexOf(groupOrder, group_id), 1);
