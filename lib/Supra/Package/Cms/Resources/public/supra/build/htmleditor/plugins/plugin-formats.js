...
YUI().add('supra.htmleditor-plugin-formats', function (Y) {
	...
	Supra.HTMLEditor.addPlugin('formats', defaultConfiguration, {
		...
		exec: function (data, command) {
			...
			//Find element
			element = htmleditor.getSelectedElement(this.formats.join(','));
			
			//Change to P, H1, H2, ...
			if (element) {
				// Change manualy, because browsers don't do this correctly
				if (element.tagName.toUpperCase() != command.toUpperCase()) {
					//History new state if we are changing tag
					history.pushState();
					
					//Create new tag
					replacement = doc.createElement(command);
					replacement.className = element.className;
					
					attr = element.getAttribute('align');
					if (attr) {
						replacement.setAttribute('align', attr);
					}
					
					htmleditor.insertBefore(replacement, element);
					...
					if (selection.start === element) {
						selection.start = replacement;
					}
					if (selection.end === element) {
						selection.end = replacement;
					}
