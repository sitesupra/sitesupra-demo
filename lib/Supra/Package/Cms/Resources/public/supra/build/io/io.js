YUI().add("supra.io", function (Y) {
	...
	var ERROR_INVALID_RESPONSE = 'Error occured, please try again later',
		ERROR_FORBIDEN         = 'You don\'t have permission';
	...
	Supra.io = function (url, cfg, permissions, callback, context) {
		...
		cfg.on.failure = function (transaction, response) {
			...
			var error_name = (response.status == 403 ? 'request_error_403' : 'request_error'),
				error_message = (Supra.Intl ? Supra.Intl.get(['error', error_name]) : '') || ERROR_FORBIDEN;
	...
	Supra.io.handleResponse = function (cfg, response) {
		if (response.status == 401) {
			...
			// Redirect if session ID has changed -> user has changed
			var key = Supra.data.get("sessionName") || 'SID',	//Cookie key
				value = key ? Y.Cookie.get(key) : "";			//Session ID
			
			if (value !== Supra.data.get("sessionId")) {
				document.location = Supra.Manager.Loader.getStaticPath();
				return;
			}
