	...
	var C = {
		...
		'compileVar': function (variable, output) {
			var _variable = variable = variable.trim(),
				compileVarReplaceModifier = C.compileVarReplaceModifier;
			
			//Extract strings
			var strings = variable.match(REG_VAR_STRING),
				i = 0,
				len = strings ? strings.length : 0;
			
			for(; i<len; i++) variable = variable.replace(strings[i], '\'$_' + i + '\'');
			...
			while (limit && REG_CHECK_MODIFIERS.test(variable)) {
				variable = variable.replace(REG_VAR_MODIFIERS, compileVarReplaceModifier);
			...
			//Restore strings
			for(i=0; i < len; i++) {
				variable = variable.replace('\'$_' + i + '\'', strings[i]);
			}
		...
		compileVarReplaceModifier: function (all, variable, filter, params_, params) {
		...
		'filters': {
			...
			'keys': function (obj) {
				...
				if (FN_IS_ARRAY(obj)) {
					...
					for(i=0, ii=obj.length; i<ii; i++) keys[i] = i;
					return keys;
				} else if (typeof obj == 'object') {
					for(i in obj) keys[len++] = i;
					return keys;
				} else {
					return [];
				}
				...
				if (typeof o2 == 'object') {
					if (FN_IS_ARRAY(o2)) {
						if (dest_is_array) {
							...
							for(i=0, ii=o2.length; i<ii; i++) dest[i] = o2[i];
						}
					} else {
						if (dest_is_array) {
							for(i in o2) dest.push(o2[i]);
						} else {
							for(i in o2) dest[i] = o2[i];
						}
					}
				}
