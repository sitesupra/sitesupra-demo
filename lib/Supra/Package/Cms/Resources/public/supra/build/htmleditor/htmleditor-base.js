YUI().add('supra.htmleditor-base', function (Y) {
	...
	HTMLEditor.CLASS_NAME = HTMLEditor.CSS_PREFIX = 'su-' + HTMLEditor.NAME;
	...
	Y.extend(HTMLEditor, Y.Base, {
		...
		bindUI: function () {
			...
			this.events.push(
				doc.after('mouseup', this._handleNodeChange, this)
			);
			
			this.events.push(
				doc.after('click', this._handleNodeChange, this)
			);
		...
		renderUI: function () {
			...
			this.attrDisabledSetter(this.get('disabled'));
		...
		render: function () {
			...
			if (!this.isNodeVisible(this.get('srcNode'), true /* check only classnames */)) {
				// Node is not visible, delay initialization
				Y.later(1500, this, this.renderDelayed);
			} else {
				// For performance reasons we might want to delay
		 ...
		 * @private
		 */
		attrDisabledSetter: function (value) {
			if (value) {
		...
		_handleNodeChange: function (event, force) {
			...
			if (event && event.type && (event.type == 'mouseup' || event.type == 'click')) {
				if (event._event.defaultPrevented) {
					// Default behaviour was prevented, ignore
					return;
				}
				...
				if (node && node.closest(this.get('srcNode'))) {
