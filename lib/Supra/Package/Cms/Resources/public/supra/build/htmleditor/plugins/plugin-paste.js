YUI().add('supra.htmleditor-plugin-paste', function (Y) {
	...
	Supra.HTMLEditor.addPlugin('paste', defaultConfiguration, {
		...
		onPaste: function (_event) {
			...
			//If there's no selection (cursor not inside editor), then skip
			if (!selection || !selection.start) return this.cancelEvent(event);
			
			if (history) history.pushTextState();
		...
		afterPasteSplitInsert: function (nodes) {
			...
			insert = Y.bind(function () {
				...
				if (parent) {
					if (parent === srcNode || (parent.tagName == 'LI' && dom[0].tagName != 'LI')) {
				...
				while (first_element !== srcNode && first_element && first_element.previousElementSibling) {
				...
				while (last_element !== srcNode && last_element && last_element.nextElementSibling) {
				...
				if (parent && parent !== srcNode && parent.parentNode && htmleditor.isNodeEmpty(parent)) {
		 ...
		 * @private
		 */
		removePastePlaceholder: function () {
