	...
	Image.parse = function (value) {
		...
		if 	(value && typeof value === 'object') {
			if (!('crop_left') in value) {
				...
				} else if (value && value.image && value.image.sizes) {
					return {
						'image': value.image,
						'crop_left': value.crop_left || 0,
						'crop_top': value.crop_top || 0,
						'crop_width': value.image.sizes.original.width,
						'crop_height': value.image.sizes.original.height,
						'size_width': value.image.sizes.original.width,
						'size_height': value.image.sizes.original.height
					};
				}
			} else {
				return value;
	...
	Image.format = function (_value) {
		if (_value && typeof _value === 'object') {
			...
			if ('crop_left' in value) {
				...
				data = value;
			} else if (value.image && 'crop_left' in value.image) {
				// InlineImage, BlockBackground
				data = value.image;
			}
			
			if (data) {
				// Make sure crop and size values are set
				if (!data.size_width) data.size_width = Supra.getObjectValue(data, ['image', 'sizes', 'original', 'width']) || 0;
				if (!data.size_height) data.size_height = Supra.getObjectValue(data, ['image', 'sizes', 'original', 'height']) || 0;
				if (!data.crop_left) data.crop_left = 0;
				if (!data.crop_top) data.crop_top = 0;
				if (!data.crop_width) data.crop_width = data.size_width;
				if (!data.crop_height) data.crop_height = data.size_height;
			}
			
			// Strip image information and replace with id
			if (value.image && value.image.id) {
				// InlineMedia, HTML image data, etc.
				value.id = value.image.id;
				delete(value.image);
				has = true;
			} else if (value.image && value.image.image && value.image.image.id) {
				// InlineImage, BlockBackground
				value.image.id = value.image.image.id;
				delete(value.image.image);
				has = true;
			} else if (typeof value.image === 'string' && value.image) {
				value.id = value.image;
				delete(value.image);
				has = true;
			}
