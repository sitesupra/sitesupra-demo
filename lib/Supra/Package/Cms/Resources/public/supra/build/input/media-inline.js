YUI.add('supra.input-media-inline', function (Y) {
	...
	Y.extend(Input, Supra.Input.Proto, {
		...
		destructor: function () {
			if (this.widgets) {
				var slideshow = this.get('slideshow'),
					...
					slides = widgets.slides,
					uploader = widgets.uploader,
					key = null;
				
				if (widgets.input_image) {
					widgets.input_image.destroy(true);
				}
				if (widgets.input_video) {
					widgets.input_video.destroy(true);
				}
			
				if (widgets.delete_image) {
					widgets.delete_image.destroy(true);
				}
				if (widgets.delete_video) {
					widgets.delete_video.destroy(true);
				}
				...
				if (slideshow) {
					slideshow.removeSlide(this.get('id') + '_slide_image');
					slideshow.removeSlide(this.get('id') + '_slide_video');
		...
		renderContent: function (node, data) {
			...
			if (data && type == 'image') {
				html = '<img class="supra-image" src="' + this.get('blankImageUrl') + '" width="100%" height="220" alt="" />';
			...
			if (data && type == 'video') {
				var Input = Supra.Input.Video,
					ratio = Input.getVideoSizeRatio(data),
		...
		_getSaveValue: function () {
			...
			if (value) {
				if (value.type === 'image') {
					// Image
					value = Y.DataType.Image.format(value);
					return value ? Supra.mix({'type': 'image'}, value) : '';
				} else {
					// Video
