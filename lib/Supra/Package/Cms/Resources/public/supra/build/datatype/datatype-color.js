...
YUI.add('supra.datatype-color', function(Y) {
	...
	//Regular expressions
	var REGEX_RGB = Color.REGEX_RGB = /^rgb(a)?\((\d+)\s?,\s?(\d+)\s?,\s?(\d+)\s?(,\s?([\d\.]+))?/i,
	...
	Color.format = function (object) {
		if (typeof object === 'object' && object) {
			...
			if ('red' in object && 'green' in object && 'blue' in object) {
				if ('alpha' in object && object.alpha != 1) {
					return 'rgba(' + object.red + ', ' + object.green + ', ' + object.blue + ', ' + object.alpha + ')';
				} else {
					// Full opacity, represent as hex
	...
	/**
	 * Compare if two colors are the same
	 * 
	 * @param {Object|String} a First color
	 * @param {Object|String} b Second color
	 * @returns {Boolean} True if both colors are the same, otherwise false
	 */
	Color.compare = function (a, b) {
		...
		if (!a && !b) {
			// Both values are invalid colors
			return false;
		} else if (a && b) {
			...
			// One of the values is not a valid color
			return false;
		}
	};
	...
	Color.math = {
		...
		'diff': function (a, b) {
			...
			if (a && b) {
				return {'red': a.red - b.red, 'green': a.green - b.green, 'blue': a.blue - b.blue, 'alpha': a.alpha - b.alpha};
			} else {
		...
		'multiply': function (a, b) {
			a = Color.parse(a);
			b = (typeof b === 'number' ? {'red': b, 'green': b, 'blue': b, 'alpha': 1} : Color.parse(b));
		...
		'normalize': function (object, clone) {
			...
			if ('red' in object && 'green' in object && 'blue' in object) {
				...
				object.alpha = Math.min(1,   Math.max(0, object.alpha));
			} else {
				//HSB
		...
		'grayscale': function (color) {
			...
			color.saturation = 0;
			color = Color.convert.HSBtoRGB(color);
