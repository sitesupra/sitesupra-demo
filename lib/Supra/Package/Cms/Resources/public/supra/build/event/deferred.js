...
YUI.add('supra.deferred', function (Y) {
	...
	/**
	 * To allow instanceof correct use we will create promises from empty object
	 * and Deferred will extend Promise
	 */
	function Promise () {};
	...
	function Deferred () {
		...
		if (arguments.length) {
			this.then.apply(this, [].splice.call(arguments, 0));
		}
	}
	
	Deferred.prototype = Supra.mix(new Promise(), {
		/**
		...
		promise: function (dest) {
			if (dest || !this._promise) {
				...
				if (!(promise instanceof Promise)) {
					Object.defineProperty(promise, '_deferredPromise', {
						enumerable: false,
						configurable: false,
						writable: false,
						value: true
		...
		then: function (done, fail, progress, context) {
			if (done instanceof Supra.Deferred) {
				return this.bind("resolved", function () { done.resolveWith(this, arguments); }, fail)
						   .bind("rejected", function () { done.rejectWith(this, arguments);  }, fail)
						   .bind("notify",   function () { done.notifyWith(this, arguments);  }, fail);
			} else {
	...
	/**
	 * Check if argument is valid deferred promise
	 * 
	 * @param {Object} promise Object to check
	 * @returns {Boolean} True if object is valid deferred promise or deferred object, otherwise false
	 */
	Deferred.isPromise = function (promise) {
		if (promise instanceof Promise || promise._deferredPromise) {
			return true;
		} else {
			return false;
		}
	 ...
	 *     })
	 */
	Deferred.when = Supra.when = function () {
				...
				if (args[i] && Y.Lang.isFunction(args[i].then)) {
					...
					(function (index, src) {
						src.then(function () {
							...
							results[index] = [].splice.call(arguments, 0);
							waiting--;
							if (!waiting) deferred.resolve(results);
						}, function () {
