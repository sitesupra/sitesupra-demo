		...
		_addItem: function (data, animate) {
			...
			if (property) {
				if (property.type === 'Set' || property.type === 'Collection') {
					...
					inputNode = targetNode ? targetNode.one('#' + targetNode.getAttribute('id') + '_' + property.id) : null;
				}
				
				console.log('COLLECTION:', property.id, inputNode ? inputNode.getDOMNode() : null);
