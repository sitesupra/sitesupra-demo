YUI.add('supra.input-collection', function (Y) {
	...
	Input.CLASS_NAME = 'su-input-collection';
	...
	Y.extend(Input, Supra.Input.Proto, {
		...
		/**
		 * List of item inputs, array of objects
		 * @type {Array}
		 * @private
		 */
		_inputs: null,
		...
		renderUI: function () {
			...
			if (!this.get('separateSlide')) {
				...
				console.log('Same slide', button.get('boundingBox').getDOMNode());
			} else {
				this._renderSlide();
				button.render(this._slideContent);
				console.log('New slide', button.get('boundingBox').getDOMNode());
			}
		...
		_addItem: function (data, animate) {
			...
			for (; i<count; i++) {
				...
				console.log('COLLECTION:', id, targetNode ? '#' + targetNode.getAttribute('id') + '_' + id : null);
		 ...
		 * 
		 * @param {Number} index Item index
		 * @returns {Object} List of all widgets for item
		 */
		...
		_getValue: function (value) {
			...
			if (!this.get('rendered') || !this._valuesRendered) {
				if (value.length && value[0] && this.SINGLE_PROPERTY_MODE) {
					for (key in value[0]) {
						return [value[0][key]];
					}
				} else {
			...
			for (; i<ii; i++) {
				...
				for (k=0; k<kk; k++) {
					...
					if (this.SINGLE_PROPERTY_MODE) {
						item = inputs[i][id].get('saveValue');
						break;
					} else {
		 ...
		 */
		_setAttrProperties: function (properties) {
			if (Array.isArray(properties)) {
				this.SINGLE_PROPERTY_MODE = false;
				return properties;
			} else if (properties && typeof properties === 'object') {
				this.SINGLE_PROPERTY_MODE = true;
				return [properties];
			} else {
				return [];
			}
