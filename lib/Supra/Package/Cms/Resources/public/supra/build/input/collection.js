	...
	Y.extend(Collection, Supra.Input.Proto, {
		...
		_addItem: function (data, animate) {
			...
			if (property) {
				...
				config = Supra.mix({}, property, {
					...
					// For 'Set' input
					'separateSlide': (property.type === 'Collection' ? true : false)
				});
		...
		_setValue: function (value) {
			...
			if (Supra.Input.isInline(property)) {
				...
				// @TODO We probably should recreate only if inputs are not valid anymore!
			} else {
				// 
				var old_count = this._count,
					count = value.length,
					i = 0;
				...
				for (; i < count; i++) {
					if (widgets[i]) {
						// Update item
						widgets[i].input.set('value', value[i]);
