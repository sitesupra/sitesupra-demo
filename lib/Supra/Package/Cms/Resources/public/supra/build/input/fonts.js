YUI.add('supra.input-fonts', function (Y) {
	...
	Y.extend(Input, Supra.Input.Proto, {
			...
			if (this.get('separateSlide')) {
				...
				if (slideshow) {
					...
					var labelNode = this.get('labelNode'),
						...
						container = this.widgets.separateContainer = Y.Node.create('<div class="yui3-widget su-input"></div>');
		...
		_renderFontGroup: function (group) {
			...
			// Slide content
			node = Y.Node.create('<div class="su-input-font-list" style="height: ' + (group.fonts.length * this.FONT_ITEM_HEIGHT) + 'px;"></div>');
