...
YUI().add('supra.htmleditor-plugin-styles', function (Y) {
	...
	Supra.HTMLEditor.addPlugin('styles', defaultConfiguration, {
		...
		getSelectors: function () {
			...
			//Text noeds can changed be changed between P, H1, H2, H3, H4 and H5
		 ...
		 */
		updateStylesList: function (targetGroups) {
			var groups = GROUPS,
				tags = {},
				tagsArr = [],
				i = 0,
			...
			for(var i=0,ii=groups.length; i<ii; i++) {
				if (groups[i].id in targetGroups) {
					Supra.mix(tags, groups[i].tagsObject);
					tagsArr = tagsArr.concat(groups[i].tags);
				}
			}
			...
			if ('text' in targetGroups) {
				...
				if (!matchesObject['H5']) matches.push({'tag': 'H5', 'main': false, 'classname': '', 'matches': selectors['H5'] || []});
			}
		...
		handleNodeChange: function (event) {
			var htmleditor = this.htmleditor,
				...
				selectedNode = htmleditor.getSelectedElement(),
				node = selectedNode,
			...
			if (htmleditor.getSelectedElement('svg,img')) {
				...
				while(node && !srcNode.compareTo(node)) {
					...
					tagNames = htmleditor.getNodeTagName(node);
			...
			if ('' in selectors && !srcNode.compareTo(selectedNode)) {
				tagNames = htmleditor.getNodeTagName(selectedNode);
			...
			*/
			htmleditor.get('toolbar').getButton('style').set('disabled', !allowEditing);
			
			this.updateStylesList(groups);
		},
		...
		updateStyle: function (e) {
			...
			var htmleditor  = this.htmleditor,
				...
				item		= e.target.closest('.style-item'),
				tag			= item.getAttribute('data-tag'),
			...
			if (!target) {
				target = htmleditor.getSelectedElement();
				target = target ? htmleditor.closest(target, 'H1, H2, H3, H4, H5, P, LI') : null;
				...
				if (target && target.tagName == 'LI') {
					...
					//Create P element
					htmleditor.exec((tag || 'p').toLowerCase());
					htmleditor.refresh(true);
					
					//Style newly created element
					if (classname) {
						var node = htmleditor.getSelectedElement();
						if (node) {
				...
				if (changeTag) {
					...
					htmleditor.exec(tag.toLowerCase());
					htmleditor.refresh(true);
