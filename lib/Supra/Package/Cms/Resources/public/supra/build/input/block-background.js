YUI.add("supra.input-block-background", function (Y) {
	...
	Input.ATTRS = {
		...
		/**
		 * Allow changing image using drag and drop from media library
		 */
		"mediaLibraryDndEnabled": {
			value: false
		}
	};
	...
	Y.extend(Input, Supra.Input.Proto, {
		...
		startEditing: function () {
			var imageResizer = this.widgets.imageResizer,
				block = this.get("root"),
				node = this.getTargetNode() || (block && block.getNode ? block.getNode().one("*") : null),
		...
		openMediaSidebar: function () {
			...
			mediasidebar.execute({
				...
				"hideToolbar": true,
				"item": path,
				"dndEnabled": this.get('mediaLibraryDndEnabled')
			});
		...
		insertImage: function (data, preventEditing) {
			var value = {
				"classname": "_custom",
				"image": Y.DataType.Image.parse(data.image)
			};
				...
				if (!this.uiControlsRendered) {
					...
					if (separate) {
						slideshow.on("slideChange", function (evt) {
							if (evt.prevVal != evt.newVal && evt.prevVal == slide_id && this.widgets.imageResizer) {
		 ...
		 * @protected
		 */ 
		_uiSync: function (_value) {
			this._uiFrozen = true;
			
			var value = (_value === undefined || _value === null ? "" : _value),
