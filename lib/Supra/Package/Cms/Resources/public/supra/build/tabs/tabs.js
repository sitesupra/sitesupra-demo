YUI.add('supra.tabs', function(Y) {
	...
	Tabs.NAME = 'Tabs';
	Tabs.CLASS_NAME = Tabs.CSS_PREFIX = 'su-' + Tabs.NAME;
	...
	Tabs.HTML_PARSER = {
		'nodeButtonContainer': function (srcNode) {
			var node = srcNode.one('.su-tabs-buttons');
			if (!node) {
				node = Y.Node.create('<div class="su-tabs-buttons"></div>');
		...
		'nodeContentContainer': function (srcNode) {
			var className = this.getClassName('contents'),
				node = srcNode.one('.su-tabs-contents');
			if (!node) {
				node = Y.Node.create('<div class="su-tabs-contents"></div>');
				this.get('contentBox').append(node);
			}
	...
	Y.extend(Tabs, Y.Widget, {
		...
		renderUI : function() {
			...
			if (attrClassName != Tabs.CLASS_NAME) {
				...
				nodeButtonContainer.removeClass('su-tabs-buttons');
				nodeButtonContainer.addClass(this.getClassName('buttons'));
				nodeContentContainer.removeClass('su-tabs-contents');
