YUI().add('supra.imageresizer', function (Y) {
	...
	Y.extend(ImageResizer, Y.Base, {
		...
		documentClick: function (e) {
			var image = this.get('image');
			if (this.get('autoClose') && image && e.target && !e.target.closest('span.supra-icon, span.supra-image, span.supra-image-resize, .supra-background-editing')) {
				this.set('image', null);
			}
		},
		...
		showContentOverlay: function () {
			...
			if (!overlay) {
				overlay = Y.Node.create('<div class="su-box-reset ' + ImageResizer.CLASS_NAME + '-overlay"></div>');
