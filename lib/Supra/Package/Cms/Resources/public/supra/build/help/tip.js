YUI().add('supra.help-tip', function (Y) {
	...
	Y.extend(Note, Y.Widget, {
		...
		_checkButtonPermissions: function (button, config) {
			var action = config.permissions,
				action_id = null,
				action_fn = null,
			...
			if (action_fn) {
				if (action.NAME) {
					if (action.get('executed')) {
						//Call function
						action[action_fn](config.id, config).done(function () {
							button.show();
							self._nodeButtons.removeClass('hidden');
						});
					} else {
						//Call after action is executed
						action.once('executedChange', function (e) {
		...
		_uiSetButtons: function (buttons) {
			...
			for (i=0, ii=buttons.length; i<ii; i++) {
				...
				if (buttons[i].permissions) {
					widget.hide();
					this._checkButtonPermissions(widget, buttons[i]);
				} else {
					node.removeClass('hidden');
				}
