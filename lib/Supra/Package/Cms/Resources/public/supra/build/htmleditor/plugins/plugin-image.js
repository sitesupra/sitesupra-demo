	...
	Supra.HTMLEditor.addPlugin("image", defaultConfiguration, {
		...
		removeSelectedImage: function () {
			if (this.selected_image) {
				...
				var image = this.selected_image,
					...
					history = this.htmleditor.getPlugin('history'),
					classname = this.configuration.wrapperClassName;
		...
		editImage: function () {
			...
			if (image) {
				...
				if (!resizer) {
					...
					// Stop editing previous image
					resizer.set("image", null);
				}
				
				//Find content width
				max_crop_width = Math.min(size.width, this.getContainerMaxWidth());
		...
		insertImage: function (event) {
			...
			if (!htmleditor.get("disabled") && htmleditor.isSelectionEditable(htmleditor.getSelection())) {
				...
				//Save history state
				var history = htmleditor.getPlugin('history');
				if (history) {
					history.pushState();
				}
			}
		...
		onNodeChange: function () {
			var htmleditor = this.htmleditor,
				element = htmleditor.getSelectedElement("img"),
		...
		updateImageSource: function (e) {
			...
			if (item_data) {
				data_object = Manager.getAction("MediaLibrary").medialist.get("data");
				image_data = data_object.cache.one(image_id);
		...
		onDrop: function (e) {
			...
			//Only if dropped from gallery, not from desktop or somewhere else
		...
		untagHTML: function (html, data) {
			...
			html = html.replace(/\{supra\.image id="([^"]+)"}/ig, function (tag, id) {
				...
				if (src) {
					item.crop_left = item.crop_left || 0;
					item.crop_top = item.crop_top || 0;
					item.crop_width = item.crop_width || (item.size_width - item.crop_left);
					item.crop_height = item.crop_height || (item.size_height - item.crop_top);
		...
		processData: function (id, data) {
			console.log(data, '->', Y.DataType.Image.format(data));
			return Y.DataType.Image.format(data);
		}
	});
