...
YUI.add("supra.input-keywords", function (Y) {
	...
	Input.ATTRS = {
		...
		// Message when autocomplete didn't had any results
	...
	Input.HTML_PARSER = {
		...
		'autoCompleteUrl': function (srcNode) {
			var value = srcNode.getAttribute('data-auto-complete-url');
			if (value) {
				return value;
			}
		},
		'autoCompleteEmptyMessage': function (srcNode) {
			var value = srcNode.getAttribute('data-auto-complete-empty-message');
			if (value) {
				return value;
			}
		},
	...
	Y.extend(Input, Supra.Input.Proto, {
		...
		renderUI: function () {
			...
			if (this.get('autoCompleteUrl')) {
				this._uiAutoCompleteUrlChange();
			}
		},
		...
		bindUI: function () {
			...
			//After autocomplete url change update autocomplete plugin
			this.after('autoCompleteUrlChange', this._uiAutoCompleteUrlChange, this);
		...
		loadItems: function () {
			...
			if (uri) {
				...
				this.suggestions = [].concat(this.get('suggestions') || []);
			...
			if (status && data && data.length) {
				...
				if (values) {
					...
					for(var j=0; j<=values.length-1; j++) {
						value = values[j];
						node = this.get('suggestionsListNode').one('span[data-value="' + value + '"]');
						if (node) {
		...
		hideSuggestion: function (suggestion) {
			...
			var escaped = Y.Escape.html(suggestion),
				node = this.get('suggestionsListNode').one('span[data-value="' + escaped + '"]'),
		...
		addItem: function (value) {
			...
			if (value.split(/\s+/).length > 5) {
				// Not more than 5 words, #5626
				value = value.split(/\s+/).slice(0, 5).join(' ');
			}
		...
		/**
		 * Handle autoCompleteUrl attribute change
		 *
		 * @private
		 */
		_uiAutoCompleteUrlChange: function () {
			var url = this.get('autoCompleteUrl');
			
			if (!this.autocomplete && url) {
				if (Supra.Input.PluginAutoComplete) {
					this.plug(Supra.Input.PluginAutoComplete, {
						'inputNode': this.get('inputNode'),
						'targetNode': this.get('inputListNode'),
						'values': url,
						'messageNoResults': this.get('autoCompleteEmptyMessage')
					});
					
					this.autocomplete.on('add', this._handleAutoCompleteItemAdd, this);
				} else {
					// Not loaded yet
					Supra('supra.input-plugin-autocomplete', Y.bind(function () {
						this._uiAutoCompleteUrlChange();
					}, this));
				}
			} else if (this.autocomplete) {
				this.autocomplete.set('values', url);
			}
		},
		
		/**
		 * Add keyword from auto complete list
		 * 
		 * @private
		 */
		_handleAutoCompleteItemAdd: function (e) {
			if (this.get('disabled')) return;
			var item = e.data;
			if (item) {
				this.get('inputNode').set('value', '');
			...
			if (Y.Lang.isArray(value)) {
				...
				for (; i<ii; i++) {
					...
					if (typeof value[i] === 'string') {
						...
						if ('title' in val) {
							val = Y.Lang.trim(val.title);
						} else if ('name' in val) {
							val = Y.Lang.trim(val.name);
						}
