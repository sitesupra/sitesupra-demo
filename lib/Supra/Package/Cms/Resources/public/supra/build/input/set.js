			...
			if (ii) {
				...
				for (i=0; i<ii; i++) {
					if (!this.getProperty(props[i].id, prevProps)) {
						...
						if (props[i].type === 'Set' || props[i].type === 'Collection') {
							inputNode = targetNode;
						} else {
							inputNode = targetNode ? targetNode.one('#' + targetNode.getAttribute('id') + '_' + id) : null;
						}
						
						//console.log('SET:', id, inputNode ? inputNode.getDOMNode() : null);
						...
						// console.log('Rendered', input.get('boundingBox').getDOMNode(), 'into', container.getDOMNode());
						// @TODO Fix order
