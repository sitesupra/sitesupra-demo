...
YUI.add("supra.datagrid", function (Y) {
	...
	Y.extend(DataGrid, Y.Widget, {
		...
		'requestParams': {
			 ...
			 */
			'toString': function () {
				return Supra.io.serializeIntoString(this.data);
			}
		},
		...
		'renderUI': function () {
			...
			for(var i=0,ii=columns.length; i<ii; i++) {
				...
				classname = column.id.replace(/[^a-z0-9\-_]*/ig, '');
				classname = classname ? 'col-' + classname.toLowerCase() : '';
				
				node = Y.Node.create('<th ' + (column.width ? 'width="' + column.width + '" ' : '') + 'class="' + classname + '" data-column-id="' + column.id + '"><span>' + (column.title || '') + '</span></th>');
				heading.append(node);
			}
		...
		'bindUI': function () {
			...
			this.tableBodyNode.delegate('mouseenter', this._handleRowEnter, 'td', this);
			this.tableBodyNode.delegate('mouseleave', this._handleRowLeave, 'td', this);
		...
		'_handleRowClick': function (e) {
			...
			if (row_id) {
				var row = this.getRowByID(row_id),
					data = row.getData(),
					column_id = row.getColumnIdFromNode(e.target);
				
				this.fire('row:click', {'data': data, 'row': row, 'element': e.target, 'column_id': column_id});
			}
		},
		
		'_handleRowEnter': function (e) {
			if (this.get('disabled')) return;
			...
			if (row_id) {
				...
				this.fire('row:enter', {'data': data, 'row': row, 'element': e.target, 'column_id': column_id});
			}
		},
		
		'_handleRowLeave': function (e) {
			if (this.get('disabled')) return;
			...
			if (row_id) {
				...
				this.fire('row:leave', {'data': data, 'row': row, 'element': e.target, 'column_id': column_id});
			}
		},
