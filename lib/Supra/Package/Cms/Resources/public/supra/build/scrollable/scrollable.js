YUI.add('supra.scrollable', function (Y) {
	...
	Y.extend(Scrollable, Y.Widget, {
		...
		bindUI: function () {
			...
			listeners.push(node.on('mousewheel', Y.bind(this.onMouseWheel, this)));
			listeners.push(node.on('scroll', this.syncUIPositionThrottled));
			
			//Drag and drop
			node = this.get('draggableNode');
			listeners.push(node.on('mousedown', this.onDragStart, this));
		...
		bindUIResize: function () {
			...
			var node = this.get('contentBox');
			listeners.push(node.on('contentresize', this.syncUIThrottledHalt, this));
			listeners.push(node.get('parentNode').on('contentresize', this.syncUIThrottled));
			
			node = node.closest('.left-container, .right-container');
			if (node) {
				listeners.push(node.on('contentresize', this.syncUIThrottled));
			}
			
			listeners.push(Y.on('resize', this.syncUIThrottled));
		},
		
		syncUIThrottledHalt: function (e) {
			// Prevent event from going up the tree, because content can't
			// overflow, so children size change can't affect parent
			e.halt();
			
			this.syncUIThrottled(e);
		},
		 ...
		 */
		syncUI: function () {
			if (this.dragging || this.get('disabled') || !this.get('rendered') || this.get('destroyed')) return;
