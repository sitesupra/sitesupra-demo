YUI.add('supra.button-group', function (Y) {
	...
	ButtonGroup.CLASS_NAME = ButtonGroup.CSS_PREFIX = 'su-' + ButtonGroup.NAME;
	...
	Y.extend(ButtonGroup, Y.Widget, {
		...
		'updateClassNames': function () {
			if (this.buttons.length) {
				var classname_first	= this.getClassName('first'),
					classname_last 	= this.getClassName('last'),
					buttons			= this.buttons,
		 ...
		 * @private
		 */
		'_uiSetSelection': function (selection) {
			var type = this.get('type');
			...
			if (this.get('rendered') && (type === 'radio' || type === 'checkbox' || type === 'radio-checkbox')) {
				...
				for (i=0, ii=selection.length; i<ii; i++) {
					selected[selection[i].id] = true;
				}
		...
		'_setSelection': function (selection) {
			...
			if (type === 'radio' || type === 'checkbox' || type === 'radio-checkbox') {
				...
				// Normalize
				for (i=0, ii=selection.length; i<ii; i++) {
					if (typeof selection[i] === 'string' || typeof selection[i] === 'number') {
						selection[i] = {
							'id': selection[i],
							'button': index[selection[i]] || null
						};
					}
				}
				
				this._uiSetSelection(selection);
			}
