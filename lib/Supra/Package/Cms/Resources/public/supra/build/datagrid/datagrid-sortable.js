	...
	Y.extend(SortablePlugin, Y.Plugin.Base, {
		...
		'setSorting': function (id, order) {
			if (this.sortingColumn != id || this.sortingOrder != order) {
				...
				if (host.get('rendered')) {
					old_node = host.get('tableHeadingNode').one('[data-column-id="' + this.sortingColumn + '"]'),
					new_node = host.get('tableHeadingNode').one('[data-column-id="' + id + '"]');
		...
		'_afterRender': function () {
			...
			for (; i<ii; i++) {
				...
				node = heading.one('[data-column-id="' + id + '"]');
