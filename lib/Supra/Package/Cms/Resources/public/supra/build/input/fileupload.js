YUI.add("supra.input-file-upload", function (Y) {
	...
	Fileupload.CLASS_NAME = Fileupload.CSS_PREFIX = 'su-' + Fileupload.NAME;
	Fileupload.ATTRS = {
		...
		'textNode': function (srcNode) {
			...
			if (!node.test(selector)) {
				...
				if (!node.test(selector)) {
					...
					node = Y.Node.create('<em class="su-input-file-upload-text"></em>');
			...
			if (node) {
				node.addClass(this.getClassName('text'));
			}
	...
	Y.extend(Fileupload, Supra.Input.Proto, {
		...
		onFileUpload: function (event) {
			...
			this.get('boundingBox').addClass(this.getClassName('uploading'));
		},
		...
		onFileAbort: function (event) {
			...
			if (!this.uploading_count) {
				...
				this.get('boundingBox').removeClass(this.getClassName('uploading'));
			}
		},
