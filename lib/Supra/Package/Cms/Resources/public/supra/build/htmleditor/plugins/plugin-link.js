YUI().add('supra.htmleditor-plugin-link', function (Y) {
	...
	Supra.HTMLEditor.addPlugin('link', defaultConfiguration, {
		...
		insertLinkConfirmed: function (data, options) {
			...
			if (data && data.href && data.href !== 'mailto:') {
				...
				if (selected_node) {
					...
					if (plugin = htmleditor.getPlugin('image')) {
						selector.push('span.' + plugin.configuration.wrapperClassName);
					}
					...
					if (selector) {
						selected_node = selected_node.closest(selector.join(', ')) || selected_node;
					}
		...
		editLink: function (event) {
			...
			if (selector.length && target && target.closest(selector.join(','))) {
				// Clicked on image or icon
				return;
			}
			
			target = event.currentTarget;
			if (!htmleditor.editingAllowed || !htmleditor.isEditable(target)) {
				return;
			}
			
			//Get current value
			var data = htmleditor.getData(target),
				classname;
			...
			if (!data) {
				...
				data = {
					...
					'resource': 'link',
					'classname': classname,
					'button': classname.indexOf(this.configuration.buttonClassName) != -1,
				};
				
				if (data.href && data.href.indexOf('mailto:') == 0) {
					data.resource = 'email';
					data.title = data.href.replace('mailto:', '');
				}
			}
		...
		editLinkConfirmed: function (data, target) {
			...
			if (data && data.href && data.href !== 'mailto:') {
				...
				htmleditor.setData(target, data, true);
