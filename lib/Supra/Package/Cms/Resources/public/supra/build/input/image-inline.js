		...
		renderUI: function () {
			...
			if (this.get('separateSlide')) {
				var buttonCustom = new Supra.Button({
					"label": Supra.Intl.get(["form", "block", value ? "change_image" : "set_image"]),
					"style": "small-gray"
				});
		...
		_setValue: function (_value) {
			...
			if (this.widgets) {
				...
				if (this.widgets.buttonCustom) {
					if (this.image) {
						this.widgets.buttonCustom.set("label", Supra.Intl.get(["form", "block", "change_image"]));
					} else {
						this.widgets.buttonCustom.set("label", Supra.Intl.get(["form", "block", "set_image"]));
					}
				}
