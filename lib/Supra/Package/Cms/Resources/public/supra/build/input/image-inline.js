YUI.add('supra.input-image-inline', function (Y) {
	...
	Y.extend(Input, Supra.Input.BlockBackground, {
		...
		startEditing: function () {
			...
			if (!imageResizer) {
				...
				imageResizer.on("resize", function (event) {
					...
					if (!imageResizer.get('image') || !imageResizer.get('image').getDOMNode()) {
						// There is no image anymore, ignore
						return;
					}
		...
		insertImage: function (data) {
			...
			if (!this.get('fixedMaxCropWidth') && container_width && container_width < 100) {
				container_width = 100;
			}
			if (!this.get('fixedMaxCropHeight') && container_height && container_height < 100) {
