YUI.add('supra.tree-node-draggable', function(Y) {
	...
	TreeNodeDraggable.CLASS_NAME = 'su-' + TreeNodeDraggable.NAME;
	TreeNodeDraggable.CSS_PREFIX = Supra.TreeNode.CLASS_NAME;
	...
	Y.extend(TreeNodeDraggable, Supra.TreeNode, {
		...
		setMarker: function (target, position, y) {
			...
			if (!this.marker) {
				marker = this.marker = Y.Node.create('<div class="su-tree-node-marker"></div>');
			} else {
