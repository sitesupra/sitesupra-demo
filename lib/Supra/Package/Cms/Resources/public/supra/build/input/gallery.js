YUI.add('supra.input-gallery', function (Y) {
	...
	Y.extend(Input, Supra.Input.Proto, {
		...
		getGalleryItemTemplate: function () {
			// @TODO Remove following line, it's for debug only
			return '<li class="pure-u pure-u-1-4"><span class="img">{{ image }}</span><h2>{{ title }}</h2></li>';
		...
		getGalleryWrapperTemplate: function () {
			// @TODO Remove following line, it's for debug only
			return '<ul class="pure-g">{{ items }}</ul>';
