Supra('supra.medialibrary-list-extended', 'supra.medialibrary-upload', function (Y) {
	...
	var TOOLBAR_BUTTONS = [
	    ...
	    }/*,
		{
			...
			'icon': '/public/cms/supra/img/toolbar/icon-media-undo.png',
			'action': 'MediaLibrary',
			'actionFunction': 'handleToolbarButton',
			'type': 'button'
	    }*/,
	...
	new Action(Action.PluginContainer, {
		...
		render: function () {
			...
			var list = this.medialist = (new Supra.MediaLibraryExtendedList({
				...
				'saveURI': Supra.Url.generate('media_library_save'),
				'moveURI': Supra.Url.generate('media_library_move'),
		...
		addKeyListeners: function () {
			...
			listeners.push(
				...
				Y.one("doc").on("key", Y.bind(function (e) {
					// If currently focus is on the input, then 'delete file' was not
					// intended action
					var focused = Supra.Y.Node(document.activeElement);
					if (focused.test('input, textarea, select')) return;
					
					// For files show confirmation message in case key is used
					var opened = this.medialist.getSelectedItem();
					if (opened && opened.type !== Supra.MediaLibraryList.TYPE_FOLDER) {
						Supra.Manager.executeAction('Confirmation', {
							'message': '{# medialibrary.confirm_delete_file #}',
							'buttons': [
								{
									'id': 'yes',
									'click': function () {
