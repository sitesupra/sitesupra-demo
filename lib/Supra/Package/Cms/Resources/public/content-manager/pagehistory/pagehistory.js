	...
	new Action(Action.PluginLayoutSidebar, {
		...
		showVersionPreview: function (e) {
			...
			if (this.current_version != version_id) {
				...
				this._loadVersionPreview(version_id, function (data, status) {
		...
		_loadVersionPreview: function (version_id, callback, context) {
			var url = Manager.Page.getDataPath('version-preview'),
				iframe = Manager.PageContent.getIframe();
			...
			Supra.io(url, {
				...
				'on': {
					'complete': function (data, status) {
						if (data) {
							iframe.set('html', data.internal_html);
						}
		...
		execute: function () {
			...
			Manager.PageContent.getContent().set('highlightMode', 'disabled');
		}
	});
