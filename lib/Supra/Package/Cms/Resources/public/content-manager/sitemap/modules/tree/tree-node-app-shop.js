...
YUI().add('website.sitemap-tree-node-app-shop', function (Y) {
	...
	Y.extend(Node, Action.TreeNodeApp, {
		...
		'openShopManager': function (tree_node, params) {
			Supra.Permission.request({'type': 'application', 'id': 'shop'}, function (permissions) {
				if (permissions.application.shop) {
					...
					Supra.Manager.executeAction('Confirmation', {
						'message': Supra.Intl.get('error.permissions').replace('{{ name }}', 'Shop application'),
						'useMask': true,
						'buttons': [
							{'id': 'error'}
						]
