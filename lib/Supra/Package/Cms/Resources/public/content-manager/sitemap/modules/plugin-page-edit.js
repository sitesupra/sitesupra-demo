...
YUI().add('website.sitemap-plugin-page-edit', function (Y) {
	...
	Y.extend(Plugin, Y.Plugin.Base, {
		...
		'_showPanel': function (e) {
			...
			//Set form values
			if (this._node.isInstanceOf('DataGridRow')) {
				this._data = e.data;
				target = this._node.node.one('.settings-icon');
			} else {
				this._data = this._node.get('data');
				target = this._node.getWidget('buttonEdit').get('boundingBox');
			}
			
			this._data = Supra.mix({}, this._data, true);
			this.setFormValues(this._data);
			
			//Panel position and style
			if (target === panel.get('alignTarget')) {
				this._widgets.panel.show();
			} else {
				hover = target.closest('.item');
				if (hover) hover.addClass('hover');
				...
				Manager.Page.updatePage(post_data, function (response, success) {
					if (success) {
						...
						if (node.isInstanceOf('TreeNode')) {
							...
							if (this._rowDataMapping[id]) {
								node.set(this._rowDataMapping[id], data[id]);
							}
						}
		...
		'_duplicatePage': function (data, status, node, post_data) {
			if (status) {
				if (node.isInstanceOf('DataGridRow')) {
					var grid = node.get('parent');
					grid.add(data, node, true);
										
					// Make sure panel is in correct position
					this._widgets.panel.syncUI();
				} else {
					this._widgets.panel.hide();
		...
		'duplicatePage': function (node) {
			...
			index = this.get('host').page_add._getTitlePathIndex(node, title, path);
			index = index ? index : 1;
