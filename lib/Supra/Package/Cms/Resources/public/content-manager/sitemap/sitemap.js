...
Supra(
	...
	'website.sitemap-tree-node-app', 'website.sitemap-tree-node-app-news', 'website.sitemap-tree-node-app-blog',
	...
	new Action(Manager.Action.PluginContainer, Action.PluginMainContent, {
		...
		handlePageMove: function (evt) {
			...
			if (reference) {
				...
				post_data = Supra.mix(post_data, {
					'reference_id': reference.get('data').id,
					'reference_type': reference_type,
				});
			} else {
				//New parent ID
				post_data.parent_id = node.get('parent').get('data').id;
			}
		...
		animate: function (node, reverse, origin) {
			...
			if (origin == 'blog') {
				...
				animationNode.setStyle('background', 'url(/public/cms/supra/img/sidebar/left-header-bg.gif) 0 0 repeat');
			} else {
				// Origin is page
