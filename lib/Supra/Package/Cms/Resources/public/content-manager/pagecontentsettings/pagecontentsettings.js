Supra(function (Y) {
	...
	new Action(Action.PluginLayoutSidebar, {
		...
		// Prevent PluginLayoutSidebar from showing this action, we will do it manually
		// because to 'execute' can be passed option 'init_only'
		PLUGIN_LAYOUT_SIDEBAR_DISABLE_AUTO_SHOW: true,
		...
		// Execute action
		execute: function (form, _options) {
			...
			var cache = (form ? this.optionsCache[form.get('id')] : null) || {},
				options = this.options = Supra.mix({
					...
					'scrollable': false,
					'title': null,
					'icon': null, //'/public/cms/supra/img/sidebar/icons/settings.png',
					
					// First init is intended to initialize PageContentSettings
					'first_init': false,
					
					// Only initialize form inside sidebar, don't show PageContentSettings
					'init_only': false
				}, cache, _options || {}),
				
				init_only = options.init_only;
			
			if (!options.first_init && !init_only) {
				//Show buttons
			...
			if (form) {
				...
				if (options.hideEditorToolbar) {
					var has_html_inputs          = options.properties ? options.properties.get('host').html_inputs_count : false,
