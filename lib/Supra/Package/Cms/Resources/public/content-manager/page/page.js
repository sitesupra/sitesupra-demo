Supra(function (Y) {
	...
	new Action({
		...
		loadPage: function (page_id) {
			...
			var iframe = Supra.Manager.PageContent.getIframe();
			if (iframe) iframe.set('loading', true);
			...
			//Load page data
			Supra.io(Supra.Url.generate('cms_pages_content_get'), {
				'data': {
					'page_id': page_id || '',
					'locale': Supra.data.get('locale')
				},
		...
		onLoadComplete: function (data, status) {
			...
			if (allow_edit) {
				//Edit button
				var button_edit = Manager.PageButtons.buttons.Root[0],
					button_unlock = Manager.PageButtons.buttons.Root[1],
				...
				if (status) {
					...
					if (data.lock && data.lock.userlogin != userlogin) {
						...
						if (!message_unlock) {
							message_unlock = Y.Node.create('<p class="su-page-butons-message"></p>');
					...
					Manager.PageToolbar.getActionButton('placeholderview').set('visible', this.getType() === 'template');
		...
		deletePage: function (page_id, locale, callback, context) {
			var uri = Supra.Url.generate('cms_pages_page_delete');
		...
		deleteVirtualFolder: function (page_id, locale, callback, context) {
			var action = Manager.getAction('VirtualFolder'),
				uri = Supra.Url.generate('cms_pages_group_delete');
		...
		duplicatePage: function (post_data, callback, context) {
			var uri = Supra.Url.generate('cms_pages_page_copy');
		...
		createPageLocalization: function (page_id, newData, source_locale, callback, context) {
			var uri = Supra.Url.generate('cms_pages_page_localization_copy');
		 ...
		 * @param {Object} context Callback function context
		 */
		updatePage: function (uri, data, callback, context) {
		...
		openSiteMap: function () {
			this.stopPageEditing().done(function () {
		...
		/**
		 * Close editing and open application if possible
		 * If current page is not part of an application then sitemap is opened
		 */
		...
		/**
		 * Check if use has required permissions to open application
		 */
		checkApplicationPermissions: function () {
			var promise = new Supra.Deferred();
			var page = Manager.Page.getPageData();
			
			if (page.application_id == 'blog') {
				Supra.Permission.request({'type': 'application', 'id': page.application_id}, function (permissions) {
					if (permissions.application[page.application_id]) {
						promise.resolve();
					} else {
						promise.reject();
					}
		...
		/**
		 * Close editing and open blog application if possible
		 * Even if current page is not part of an application tries to open it as blog app
		 */
		openBlogApplication: function () {
			this.stopPageEditing().done(function () {
				var page = Manager.Page.getPageData(),
					data = {};
				
				if (page.application_id == 'blog') {
					data.parent_id = page.application_page_id;
				}
				
				Supra.Manager.executeAction('Blog', data);
			});
		},
		
		/**
		 * Check if use has required permissions to open blog application
		 */
		checkBlogApplicationPermissions: function () {
			var promise = new Supra.Deferred();
		...
		/**
		 * Stop editing
		 * Promise is resolved when all editing actions has been closed
		 * 
		 * @returns {Object} Deferred promise
		 */
		stopPageEditing: function (deferred) {
			var deferred = (deferred && deferred instanceof Supra.Deferred ? deferred : new Supra.Deferred()),
				...
				{
					'test': function () { return Supra.Manager.getAction('PageContent').hasUnsavedChanges(); },
					'close': function () { },
					'delay': 150
				}
			];
			...
			for (var i=0,ii=tests.length; i < ii; i++) {
				...
				if (typeof test.name === 'string') {
					test = {
						'test': function () { return Supra.Manager.getAction(tests[i].name).get('visible'); },
						'close': function () { return Supra.Manager.getAction(tests[i].name).hide(); },
						'delay': test.delay || 250
					};
				}
				if (test.test()) {
					test.close();
					Y.later(test.delay, this, function () {
						this.stopPageEditing(deferred);
					});
