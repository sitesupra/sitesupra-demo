...
Supra('supra.languagebar', function (Y) {
	...
	new Action(Action.PluginContainer, {
		...
		setApplicationId: function (id) {
			...
			this.back_button.set('label', Supra.Intl.get(['page', 'back_to_sitemap']));
			
			if (id == 'blog') {
				Supra.Permission.request({'type': 'application', 'id': id}, function (permissions) {
					if (permissions.application.blog) {
						this.back_button.set('label', Supra.Intl.get(['page', 'back_to_blog']));
					}
				}, this);
			}
		},
