Supra('anim', 'transition', function (Y) {
	...
	new Action(Action.PluginLayoutSidebar, {
		...
		'onSiteMapPageRestore': function (e) {
			...
			Supra.io(Supra.Url.generate('recycle_pages_restore'), {
				'data': out,
				'method': 'post',
				'context': this,
				'on': {
					'complete': function (response, status) {
						if (status) {
							// Restore returns data about localizations, etc.
		...
		'getLoadRequestURI': function (mode) {
			...
			switch (mode) {
				case 'pages':
					return Supra.Url.generate('recycle_pages_load');
				case 'templates':
					return Supra.Url.generate('recycle_templates_load');
				default:
					throw new Error('Recycle bin mode "' + mode + '" is not supported');
			}
		},
