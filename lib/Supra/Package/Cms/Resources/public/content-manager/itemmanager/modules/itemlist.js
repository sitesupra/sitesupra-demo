YUI.add('itemmanager.itemlist', function (Y) {
	...
	Y.extend(ItemList, Y.Plugin.Base, {
		...
		/**
		 * Is being initialized for first time, not after reset
		 * @type {Boolean}
		 * @protected
		 */
		firstInit: true,
		...
		/**
		 * Item list is being initialized
		 * @type {Boolean}
		 * @protected
		 */
		initializing: true,
		...
		initializer: function(config) {
			...
			this.plug(Supra.ItemManagerItemListDrop);
			this.plug(Supra.ItemManagerItemListUploader);
		...
		/**
		 * Returns item set input
		 *
		 * @param {String} item Item id
		 * @returns {Object} input
		 */
		getItemSet: function (item) {
		...
		getItemInput: function (item, property) {
			var set = this.getItemSet(item);
			return set ? set.getInput(property) : null;
		},
		...
		addItem: function (data) {
			...
			if (data) {
				values = Supra.mix(values, data);
			}
			
			values = this.get('form').getInput('collection').addItem(values);
			return values;
		},
		...
		removeItem: function (id) {
			...
			if (set) {
				...
				// Removing by index, set names are indexes, but as strings
				index = parseInt(set.get('name'));
