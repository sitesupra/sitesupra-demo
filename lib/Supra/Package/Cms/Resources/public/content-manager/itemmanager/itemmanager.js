...
function (Y) {
	...
	new Action(Action.PluginContainer, Action.PluginMainContent, {
		...
		setupIframe: function () {
			...
			var iframe = this.widgets.iframe = new Supra.Iframe({
		...
		setupForm: function () {
			...
			button = this.widgets.deleteButton = new Supra.Button({'label': Supra.Intl.get(['buttons', 'delete']), 'style': 'small-red'});
			button.render(content);
			button.addClass('su-button-delete');
			button.on('click', this.removeSelectedImage, this); // @TODO
			*/
		 ...
		 */
		getData: function () {
			var data = this.widgets.form.getSaveValues().collection,
				i = 0,
