...
Supra.addModules({
	'itemmanager.itemlist': {
		path: 'itemlist.js',
		requires: ['supra.iframe', 'plugin', 'itemmanager.itemlist-order', 'itemmanager.itemlist-drop', 'itemmanager.itemlist-uploader']
	},
	'itemmanager.itemlist-order': {
		path: 'itemlist-order.js',
		requires: ['plugin', 'dd-delegate']
	},
	'itemmanager.itemlist-drop': {
		path: 'itemlist-drop.js',
		requires: ['plugin', 'dd-delegate']
	},
	'itemmanager.itemlist-uploader': {
		...
		processProperties: function (_properties) {
			...
			for (; i<ii; i++) {
				...
				if (type === 'InlineImage' || type === 'BlockBackground') {
					// Allow drag and drop of files from media library
					properties[i].mediaLibraryDndEnabled = true;
				}
			}
