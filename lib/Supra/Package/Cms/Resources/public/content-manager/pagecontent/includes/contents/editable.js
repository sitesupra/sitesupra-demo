		...
		afterSetHTML: function () {
			...
			if (this.properties) {
				...
				for(var i=0, ii=properties.length; i<ii; i++) {
					...
					if (is_inline || properties[i].type === 'Collection' || properties[i].type === 'Set') {
						id = properties[i].id;
				...
				if (active_inline_property) {
					...
					// Restore previous active inline property, but prefer HTML input if possible
					// This is done so that activating partial inline inputs wouldn't trigger sidebar after "Done"
					// button was already pressed
					if (!(active_inline_property in inline_inputs)) {
						active_inline_property = null;
					}
					if (!active_inline_property || !(inline_inputs[active_inline_property] instanceof Supra.Input.InlineHTML)) {
						for(id in inline_inputs) {
							if (inline_inputs[id] instanceof Supra.Input.InlineHTML) {
								active_inline_html_property = id;
								break;
							}
						}
					}
					
					if (active_inline_html_property in inline_inputs) {
						this.set('active_inline_property', active_inline_html_property);
					} else if (active_inline_property in inline_inputs) {
