YUI.add('supra.page-content-properties', function (Y) {
	...
	Y.extend(Properties, Y.Plugin.Base, {
			...
			for(var i=0, ii=properties.length; i<ii; i++) {
				...
				if (is_contained && !is_inline) {
					...
					if (properties[i].type === 'Set' || properties[i].type === 'Collection') {
						// Collection and Set needs to have targetNode so it can
						// be passed to children, which may need it
						form_config.inputs.push(Supra.mix({
							'targetNode': host_node
						}, properties[i]));
					} else {
			...
			if (property) {
				...
				if (!is_contained) {
					...
					if (id in inputs) {
						...
						//Get input config
						var group_node = this._group_nodes[property.group || 'default'];
		...
		toolbarButtonClickOpenGroup: function (button_id, button_config) {
			...
			//not the one which is currently active
