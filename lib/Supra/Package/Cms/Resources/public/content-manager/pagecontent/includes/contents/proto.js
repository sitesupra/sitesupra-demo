YUI.add('supra.page-content-proto', function (Y) {
	...
	ContentProto.CSS_PREFIX = ContentProto.CLASS_NAME = 'su-' + ContentProto.NAME;
	
	//CSS classes
	var CLASSNAME = 'su-content',
		...
		CLASSNAME_OVERLAY_HOVER = 'su-content-overlay-hover',
	...
	Y.extend(ContentProto, Y.Base, {
		...
		createChild: function (data, attrs, use_only, index) {
			var win = this.get('win'),
				...
				type = data.type,
				properties = Manager.Blocks.getBlock(type),
				classname,
				html_id = '#content_' + (data.id || null);
			
			if (!use_only || body.one(html_id)) {
				classname = type[0].toUpperCase() + type.substr(1);
		...
		fireContentEvent: function (event_name, node, data) {
			return this.get("super").get("iframe").fireContentEvent(event_name, node, data);
		},
		...
		bindUI: function () {
			...
			this.after('editingChange', this._handleEditingChange, this);
		...
		_handleEditingChange: function (e) {
			if (e.prevVal !== e.newVal) {
				if (e.newVal) {
					...
					//Prevent user from switching to other blocks
					this.get('body').addClass('su-editable');
