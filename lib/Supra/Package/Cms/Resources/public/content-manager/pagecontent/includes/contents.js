...
YUI.add('supra.iframe-contents', function (Y) {
	...
	IframeContents.CLASS_NAME = 'su-' + IframeContents.NAME;
	...
	Y.extend(IframeContents, Y.Base, {
		 ...
		 * @private
		 */
		createChildren: function (_data, use_only) {
			var data = _data || this.get('contentData');
			...
			if (data) {
				...
				for(var i=0,ii=data.length; i < ii; i++) {
					...
					html_id = '#content_' + (data[i].id || null);
		...
		_setDisabled: function (value) {
			this.get('body').toggleClass('su-editable', !value);
