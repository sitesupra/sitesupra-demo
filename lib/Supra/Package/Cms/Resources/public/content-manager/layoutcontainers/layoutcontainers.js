	...
	new Action({
		...
		bindLayouts: function () {
			...
			layoutLeftContainer.on('visibleChange', function (evt) {
				if (evt.newVal != evt.prevVal && evt.newVal && layoutRightContainer.get('visible')) layoutRightContainer.hide();
			});
			layoutRightContainer.on('visibleChange', function (evt) {
				if (evt.newVal != evt.prevVal && evt.newVal && layoutLeftContainer.get('visible')) layoutLeftContainer.hide();
			});
