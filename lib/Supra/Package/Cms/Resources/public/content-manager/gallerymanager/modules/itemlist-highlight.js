YUI.add('gallerymanager.itemlist-highlight', function (Y) {
	...
	//Template
	var TEMPLATE_HIGHLIGHT_T = '<div class="su-inline-reset su-box-reset supra-gallerymanager-highlight-top"></div>',
		TEMPLATE_HIGHLIGHT_R = '<div class="su-inline-reset su-box-reset supra-gallerymanager-highlight-right"></div>',
		TEMPLATE_HIGHLIGHT_B = '<div class="su-inline-reset su-box-reset supra-gallerymanager-highlight-bottom"></div>',
		TEMPLATE_HIGHLIGHT_L = '<div class="su-inline-reset su-box-reset supra-gallerymanager-highlight-left"></div>';
	...
	Y.extend(ItemListHighlight, Y.Plugin.Base, {
		...
		showHighlight: function (target, showMessage) {
			var iframe = this.get('host').get('iframe');
			var doc = iframe ? iframe.get('doc') : null;
			...
			if (!this.target) {
				return
			} else if (!Y.DOM.inDoc(this.target.getDOMNode())) {
				// Element was removed
				this.hideHighlight();
				return;
			}
			...
			region.top -= borderTop + offset - paddingTop;
			region.left -= borderLeft + offset - paddingLeft;
			...
			if (showMessage) {
				...
				nodes.item(2).set('innerHTML', '<span class="su-inline-reset su-box-reset">' + Supra.Intl.get(['gallerymanager', 'edit_move']) + '<span class="su-inline-reset su-box-reset"></span></span>');
