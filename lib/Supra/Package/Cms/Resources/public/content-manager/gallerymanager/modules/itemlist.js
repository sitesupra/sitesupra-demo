YUI.add('gallerymanager.itemlist', function (Y) {
	...
	var TEMPLATE_ADD_NEW_ITEM = '\
			<span class="su-inline-reset su-box-reset">\
				<span class="supra-gallerymanager-new-empty-label su-inline-reset su-box-reset">' + Supra.Intl.get(['gallerymanager', 'empty']) + '</span>\
				<span class="supra-gallerymanager-new-add su-inline-reset su-box-reset">\
					<span></span>\
					' + Supra.Intl.get(['gallerymanager', 'add_item']) + '\
				</span>\
				<span class="supra-gallerymanager-new-drop su-inline-reset su-box-reset">\
					<span></span>\
					' + Supra.Intl.get(['gallerymanager', 'drag_and_drop']) + '\
				</span>\
				' + (FILE_API_SUPPORTED ? '\
						<span class="supra-gallerymanager-new-drop-add su-inline-reset su-box-reset">\
	...
	Y.extend(ItemList, Y.Plugin.Base, {
		...
		replaceItem: function (id, image) {
			...
			for (var i=0, ii=properties.length; i<ii; i++) {
				...
				if (property === 'title' && old_data.properties[property] === old_data.image.image.filename) {
					// Old title was the same as image title, don't copy old title into data
				} else if (old_data.properties[property] && old_data.properties[property] != properties[i].value) {
		...
		blurInlineEditor: function () {
			if (this.editingId) {
				...
				this.fire('blurItem', {'input': this.editingInput, 'id': id, 'property': this.editingProperty});
				...
				// Remove new item if item data is invalid
				if (this.newItemImage) {
					if (!this.newItemImage || !this.items[this.newItemImage.id] || !this.items[this.newItemImage.id].image) {
						this.removeItem(this.newItemImage.id);
					}
		...
		renderIframe: function () {
			var host   = this.get('host'),
 			...
 			iframe.render(host.one('.su-gallery-manager-content'));
			...
			if (node) {
				...
				while (node && node.tagName) {
					...
					if (nodeTag == 'body') {
						...
						structure.unshift('<div class="su-inline-reset su-box-reset su-gallerymanager-wrapper">');
						structure.push('</div>');
					}
				...
				structure = [
					'<html class="supra-cms">',
						'<body><div class="su-inline-reset su-box-reset su-gallerymanager-wrapper">',
							'<ul>',
							'</ul>',
						'</div></body>',
					'</html>'
		...
		getHTMLLinks: function () {
			...
			for (; s<ss; s++) {
				styleType = styles.item(s).getAttribute('type');
		...
		getItemRenderData: function (data) {
			...
			if (data) {
				...
				for (key in model.property) {
					model.property[key] = '<span class="su-inline-reset" data-supra-item-property="' + key + '">' + (model.property[key] || '') + '</span>';
				}
			}
		...
		getOriginalDocument: function () {
			return Action.getIframe().get('doc');
		},
