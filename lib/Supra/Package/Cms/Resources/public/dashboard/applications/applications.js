	...
	new Action({
		...
		renderHeader: function () {
			...
			if (Supra.data.get(["application", "id"]) === "cms_dashboard") {
				Supra.Y.one("div.su-app-content").addClass("hidden");
			}
...
//			}
		...
		hide: function () {
			...
			if (Y.UA.gecko || Y.UA.opera || Y.UA.ie) {
				...
				//or browsers with performance issue
				transition = {
					"opacity": 0,
					"duration": 0.35
				};
		...
		show: function () {
			...
			Y.later(150, this, function () {
				...
				if (transition) {
					...
					// We  use timer, because in FF callback will not execute ever again
					// if we stop animation in the middle
					Y.later(500, this, after);
				} else {
					after();
				}
			});
